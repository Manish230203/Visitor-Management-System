...
def live_visitors():
    ...
    for r in rows:
        ...
        # Overstay logic (2 hours)
        if check_in_time and datetime.now() - check_in_time > timedelta(hours=2):
            status = "Overstay"
...
def search_visitors():
    ...
    cursor.execute("""
        ...
        FROM visitor_requests vr
        JOIN visitors v ON vr.visitor_id = v.visitor_id
        JOIN employees e ON vr.host_id = e.employee_id
...
SELECT
    ...
    INITCAP(REPLACE(SPLIT_PART(u.email,'@',1),'',' ')) AS approver_name,
