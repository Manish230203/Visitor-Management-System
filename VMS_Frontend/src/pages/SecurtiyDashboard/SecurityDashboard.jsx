...
import { Users, UserCheck, Clock, AlertTriangle, Search, CheckCircle, Camera, QrCode, Shield, UserPlus, X } from "lucide-react";
...
const SecurityDashboard = () => {
  ...
  useEffect(() => {
    if (showForm || showCamera || showQRModal) {
  ...
  }, [showForm, showCamera, showQRModal]);
                      ...
                      <div className="security-request-info">
                        ...
                        <div>
                          ...
                          <div className="security-request-meta">
                            <span>{req.company}</span> · <span>{req.purpose}</span> · <span>Host: {req.host || "Walk-in"}</span>
                          </div>
                        ...
                        <tbody>
                          {liveVisitors.filter(v =>
                            ...
                            <tr key={v.request_id}>
                              ...
                              <td>
                                <div className="security-host-name">{v.host}</div>
                                <div className="security-host-id" style={{ fontSize: '11px', color: '#64748b' }}>{v.employee_id ? `ID: ${v.employee_id}` : ""}</div>
                              </td>
                              <td><span className="security-badge-tag">{v.request_id}</span></td>
                              <td>
            ...
            {!capturedImage ? (
              <>
                <Webcam
                  ...
                  style={{ borderRadius: "12px", width: "100%", maxWidth: "320px" }}
                />
                <br />
                <button className="modal-btn capture-btn" onClick={capturePhoto}>📸 Capture</button>
              </>
            ) : (
              <>
                <img src={capturedImage} width={300} alt="preview" />
                <br />
                <button className="modal-btn capture-btn" onClick={uploadCapturedPhoto}>✅ Upload</button>
                <button className="modal-btn close-btn" onClick={() => setCapturedImage(null)}>🔄 Retake</button>
              </>
            )}

            <br />
            <button className="modal-btn close-btn" onClick={() => setShowCamera(false)}>❌ Close</button>
          </div>
        </div>
      )}
      <QRRegistrationModal 
